...
from database.models import Event, Expense, Participant
...
def expenses_list_keyboard(event_id: int, expenses: list[Expense], page: int) -> InlineKeyboardBuilder:
    ...
    start = page * EXPENSES_PAGE_SIZE
    page_items = expenses[start : start + EXPENSES_PAGE_SIZE]
    ...
    if total_pages > 1:
        # Кнопки "назад"/"вперёд" по страницам — есть смысл показывать,
        # только если трат больше одной страницы
        nav_row = []
        if page > 0:
            nav_row.append(
...
def payer_picker_keyboard(participants: list[Participant]) -> InlineKeyboardBuilder:
...
def skip_description_keyboard() -> InlineKeyboardBuilder:
    """Показывается после ввода суммы — можно написать описание текстом
    или сразу пропустить этот шаг кнопкой."""
    builder = InlineKeyboardBuilder()
    builder.button(text="Пропустить", callback_data="skip_description")
    builder.button(text="❌ Отмена", callback_data="cancel_add_expense")
    builder.adjust(1)
    return builder.as_markup()


def split_choice_keyboard() -> InlineKeyboardBuilder:
    """Выбор способа деления траты между участниками."""
    builder = InlineKeyboardBuilder()
    builder.button(text="👥 На всех участников", callback_data="split_all")
    builder.button(text="✋ Выбрать вручную", callback_data="split_custom")
    builder.button(text="❌ Отмена", callback_data="cancel_add_expense")
    builder.adjust(1)
    return builder.as_markup()
...
def expense_detail_keyboard(expense_id: int, event_id: int) -> InlineKeyboardBuilder:
    builder = InlineKeyboardBuilder()
    builder.button(text="🗑 Удалить", callback_data=f"delete_expense:{expense_id}")
    builder.button(text="⬅️ К списку трат", callback_data=f"expenses:{event_id}")
    builder.adjust(1)
    return builder.as_markup()
